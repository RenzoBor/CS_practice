...
class PlaintextMessage(Message):
    ...
    def change_shift(self, shift):
        ...
        Returns: nothing
        '''
        assert shift > 26 and shift <=0, "Shift must be between 0 and 26 (Included)"
        self.shift = shift
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(26):
            ...
            realWords=0
            for word in words:
                if is_word(validWords, word):
                    realWords+=1
                    if realWords > finalRealWordsCount:
                        bestShift=i
                        finalRealWordsCount=realWords
        return (bestShift, self.apply_shift(bestShift))
                    
def decrypt_story():
    story=get_story_string()
    storyDecrypt=CiphertextMessage(story)
    return storyDecrypt
...
#Story decryptation:
print(decrypt_story().decrypt_message())